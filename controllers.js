...
const Config = require('./config'); // Config now includes cache
const Services = require('./services');

const { User, Product, Order, BannerConfig } = Models;
const { 
    sendEmail, categories, categoryNames, 
    appCache, CACHE_TTL_SECONDS, clearProductListCaches, clearProductDetailCache // Destructure cache utilities
} = Config;
...
exports.auth_getHomePage = async (req, res, next) => {
  try {
    ...
    // --- Caching Logic for Products ---
    let productsCacheKey = 'products_home'; // Default key
    if (searchTerm) productsCacheKey = `products_list_search_${searchTerm.toLowerCase().replace(/\s+/g, '_')}`;
    else if (categoryFilter) productsCacheKey = `products_list_category_${categoryFilter.toLowerCase().replace(/\s+/g, '_')}`;
    
    let products = appCache.get(productsCacheKey);
    if (!products) {
        ...
        products = await Product.find(query, projection).sort(sort).lean();
        if (products) appCache.set(productsCacheKey, products, CACHE_TTL_SECONDS.PRODUCTS_LIST);
        console.log(`Fetched products for "${productsCacheKey}" from DB.`);
    } else {
        console.log(`Served products for "${productsCacheKey}" from cache.`);
    }

    // --- Caching Logic for Banners ---
    const bannersCacheKey = 'homepage_banners_main';
    let bannerConfig = appCache.get(bannersCacheKey);
    if (!bannerConfig) {
        bannerConfig = await BannerConfig.findOne({ configKey: 'mainBanners' }).lean();
        if (bannerConfig) appCache.set(bannersCacheKey, bannerConfig, CACHE_TTL_SECONDS.BANNERS);
        console.log(`Fetched banners from DB.`);
    } else {
        console.log(`Served banners from cache.`);
    }
...
exports.admin_uploadProduct = async (req, res, next) => {
     ...
     try {
         ...
         clearProductListCaches(); // Invalidate product list caches
         ...
         reviewProductWithGemini(newProduct).then(async reviewResult => {
             try {
                 ...
                 if (ptu.reviewStatus !== 'pending') clearProductListCaches(); // Re-clear if status changes
...
exports.admin_getManageProductsPage = async (req, res, next) => {
    try {
        // No caching for admin manage products page, want to see fresh data.
...
exports.admin_getEditProductPage = async (req, res, next) => {
    try {
        // No caching for admin edit product page.
...
exports.admin_updateProduct = async (req, res, next) => {
     ...
     try {
         ...
         clearProductListCaches(); // Invalidate general product lists

         req.flash('success_msg', `Product "${product.name}" updated by admin.`); res.redirect('/admin/manage-products');
     } catch (error) {
...
exports.admin_getManageOrdersPage = async (req, res, next) => {
     try {
         // No caching for admin manage orders, usually needs fresh data.
...
exports.admin_cancelOrderByAdmin = async (req, res, next) => {
     ...
     try {
         ...
         // If stock changes impact public views, product caches might need clearing
         order.products.forEach(item => { if (item.productId?._id) clearProductDetailCache(item.productId._id.toString()); });
         clearProductListCaches();
...
exports.admin_getManageUsersPage = async (req, res, next) => {
     try {
         // No caching for user management usually.
...
exports.admin_getManageBannersPage = async (req, res, next) => {
    try {
        const cacheKey = 'banner_config_main';
        let bannerConfig = appCache.get(cacheKey);
        if (!bannerConfig) {
            bannerConfig = await BannerConfig.findOne({ configKey: 'mainBanners' }).lean() || { configKey: 'mainBanners', banners: [] };
            if (bannerConfig) appCache.set(cacheKey, bannerConfig, CACHE_TTL_SECONDS.BANNERS);
            console.log('Fetched banner config from DB for admin page.');
        } else {
            console.log('Served banner config from cache for admin page.');
        }
...
exports.admin_updateBanners = async (req, res, next) => {
    ...
    try {
        ...
        appCache.del('banner_config_main'); // For admin page
        appCache.del('homepage_banners_main'); // For public homepage
        console.log('Cleared banner caches.');
...
exports.order_placeOrder = async (req, res, next) => {
     ...
     try {
         ...
         let productIdsInOrder = []; // For cache invalidation
         ...
         for (const item of user.cart) {
             ...
             productIdsInOrder.push(product._id.toString());
         }
         if (validationFailed) {
         ...
         productIdsInOrder.forEach(id => clearProductDetailCache(id));
         clearProductListCaches(); // Stock changed, invalidate lists
...
exports.order_cancelOrder = async (req, res, next) => { 
     ...
     try {
         ...
         let productIdsToClear = []; // For cache
         ...
         const restorePromises = order.products.map(item => {
             ...
             productIdsToClear.push(item.productId._id.toString());
         ...
         productIdsToClear.forEach(id => clearProductDetailCache(id));
         clearProductListCaches(); // Stock changed
...
exports.order_getMyOrders = async (req, res, next) => {
     try {
         // User's own orders, usually don't need aggressive caching unless very high traffic for specific user.
...
exports.order_generateAndSendDirectDeliveryOTPByAdmin = async (orderId) => { 
    try {
...
exports.product_getProducts = async (req, res, next) => {
  try {
    ...
    let cacheKey = 'products_list_all';
    if (searchTerm) cacheKey = `products_list_search_${searchTerm.toLowerCase().replace(/\s+/g, '_')}`;
    else if (categoryFilter) cacheKey = `products_list_category_${categoryFilter.toLowerCase().replace(/\s+/g, '_')}`;
    ...
    if (!products) {
        ...
        if (searchTerm) { 
            ...
            console.warn(`Invalid cat filter on /products: ${categoryFilter}`); 
        }
        products = await Product.find(query, projection).sort(sort).lean();
        if (products) appCache.set(cacheKey, products, CACHE_TTL_SECONDS.PRODUCTS_LIST);
        console.log(`Fetched products for "${cacheKey}" from DB.`);
    } else {
        console.log(`Served products for "${cacheKey}" from cache.`);
    }
...
exports.product_getProductDetails = async (req, res, next) => {
  ...
  try {
    let product = appCache.get(cacheKey);
    if (!product) {
        product = await Product.findById(productId).populate('sellerId', 'name email').lean();
        if (product) appCache.set(cacheKey, product, CACHE_TTL_SECONDS.PRODUCT_DETAIL);
        console.log(`Product ${productId} fetched from DB.`);
    } else {
    ...
    let userRating = null; 
    if (user) userRating = product.ratings?.find(r => r.userId?.toString() === user._id.toString())?.rating || null;
    
    const ratingCounts = { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 }; 
    let totalRatings = 0;
    if (product.ratings?.length > 0) { 
        totalRatings = product.ratings.length; 
        product.ratings.forEach(r => { if (r.rating && ratingCounts[r.rating] !== undefined) ratingCounts[r.rating]++; }); 
    }
    ...
    res.render('products/detail', { title: product.name, product: product, isApproved: isApproved, isAdminView: isAdmin, isOwnerView: isOwner, userRating: userRating, userCanRate: !!user, ratingCounts: ratingCounts, totalRatings: product.numReviews || totalRatings });
      ...
      if (error.name === 'CastError') { const err = new Error('Invalid Product ID'); err.status = 404; return next(err); } 
      next(error); 
  }
};
...
exports.product_rateProduct = async (req, res, next) => {
     ...
     try {
         ...
         clearProductDetailCache(productId); // Invalidate cache as rating changed

         req.flash('success_msg', 'Rating submitted!'); res.redirect(`/products/${productId}`);
...
exports.product_getProductSuggestions = async (req, res, next) => {
     ...
     const cacheKey = `suggestions_${searchTerm.trim().toLowerCase().replace(/\s+/g, '_')}`;
     let suggestions = appCache.get(cacheKey);

     if (!suggestions) {
        try {
             const regex = new RegExp(escapeRegex(searchTerm), 'i');
             const query = { $or: [ { name: regex }, { category: regex } ], reviewStatus: 'approved', stock: { $gt: 0 } };
             suggestions = await Product.find(query).select('_id name imageUrl').limit(limit).sort({ name: 1 }).lean();
             if(suggestions) appCache.set(cacheKey, suggestions, CACHE_TTL_SECONDS.SUGGESTIONS);
             console.log(`Fetched suggestions for "${searchTerm}" from DB.`);
        } catch (error) { 
            console.error("Error fetching suggestions:", error); 
            return res.status(500).json({ error: 'Failed to fetch' }); 
        }
     } else {
         console.log(`Served suggestions for "${searchTerm}" from cache.`);
     }
...
exports.user_getUserProfilePage = async (req, res, next) => {
    try {
        // User profile not typically cached aggressively due to personalization.
...
exports.user_getCart = async (req, res, next) => {
     try {
         // Cart is highly dynamic, usually not cached at this level. Session handles cart data.
...
exports.user_getCheckoutPage = async (req, res, next) => {
    try {
       // Checkout page data is also usually too dynamic/user-specific for simple caching
...
exports.user_lookupPincode = async (req, res) => {
    ...
    if (cachedData) {
        console.log(`Pincode Lookup HIT for ${pincode} from cache.`);
        return res.json(cachedData);
    }
    console.log(`Pincode Lookup MISS for ${pincode}. Fetching from API.`);
    ...
    try {
        ...
        if (result.Status !== 'Success') { 
            console.log(`Pincode ${pincode} API status: ${result.Status} msg: ${result.Message}`); 
            responseData = { success: false, message: `Pincode not found (${result.Message || 'No records'})` };
            // Don't cache negative results aggressively unless intended, or use very short TTL
            appCache.set(cacheKey, responseData, CACHE_TTL_SECONDS.PINCODE / 24); // e.g., cache not found for 1 hour
            return res.status(404).json(responseData);
        }
        if (!result.PostOffice || !Array.isArray(result.PostOffice) || result.PostOffice.length === 0) { 
            console.warn(`Pincode ${pincode} OK but no PostOffice data.`); 
            responseData = { success: true, location: { pinCode: pincode, mandalName: '', districtName: '', stateName: '', localities: [] } };
            appCache.set(cacheKey, responseData, CACHE_TTL_SECONDS.PINCODE);
        ...
        console.log(`Pincode Lookup OK ${pincode}. Loc:`, location); 
        responseData = { success: true, location };
        appCache.set(cacheKey, responseData, CACHE_TTL_SECONDS.PINCODE);
        res.json(responseData);
    } catch (error) { 
        console.error(`Pincode lookup Net/Req Error ${pincode}:`, error.message); 
        let status = 500; let msg = 'Error looking up pincode.'; 
        if (axios.isAxiosError(error)) { 
            if (error.code === 'ECONNABORTED') { msg = 'Lookup timeout.'; status = 504; } 
            else if (error.response) { msg = `API error (${error.response.status}).`; status = 502; } 
            else if (error.request) { msg = 'Net error.'; status = 502; } 
        } 
...
exports.seller_uploadProduct = async (req, res, next) => {
     ...
     try {
         ...
         clearProductListCaches(); // New product, invalidate lists
         ...
         reviewProductWithGemini(newProduct).then(async reviewResult => {
             try {
                 ...
                 if(ptu.reviewStatus !== 'pending') clearProductListCaches(); // Re-clear if status change quickly
...
exports.seller_getManageProductsPage = async (req, res, next) => {
    try {
        // Seller manage page not cached usually, wants to see own fresh data.
...
exports.seller_updateProduct = async (req, res, next) => {
      ...
      try {
          ...
          reviewProductWithGemini(product).then(async reviewResult => {
              try {
                  const ptu = await Product.findById(product._id); if (ptu) { ptu.reviewStatus = reviewResult.status; ptu.rejectionReason = reviewResult.reason; await ptu.save(); console.log(`P.ID ${product._id} review updated: ${reviewResult.status} after edit.`); if(ptu.reviewStatus !== 'pending') clearProductListCaches();}
...
exports.seller_getManageOrdersPage = async (req, res, next) => {
     try {
         // Seller orders, no caching here for freshness.
